...
let GeraewProvider = GeraewProvider_1 = class GeraewProvider {
    ...
    async startAndPollVideo(route, body, generationId, model) {
        ...
        if (!startResponse.ok) {
            ...
            const safetyError = content_safety_error_1.ContentSafetyError.fromErrorMessage(errorText);
            if (safetyError) {
                throw safetyError;
            }
    ...
    async pollVideoStatus(operationName, maxAttempts = 60, intervalMs = 10_000) {
        ...
        for (let attempt = 0; attempt < maxAttempts; attempt++) {
            ...
            if (data.error) {
                ...
                const safetyError = content_safety_error_1.ContentSafetyError.fromErrorMessage(errorStr);
                if (safetyError) {
                    throw safetyError;
                }
    ...
    sanitizeBodyForLog(body) {
        ...
        for (const [key, value] of Object.entries(body)) {
            if (key === 'first_frame' || key === 'last_frame') {
                ...
                sanitized[key] = value.map((ref) => ({
                    ...ref,
                    base64: `[base64 ${typeof ref.base64 === 'string' ? `${Math.round(ref.base64.length / 1024)}KB` : 'omitted'}]`,
                }));
            }
