...
let NanoBananaProvider = NanoBananaProvider_1 = class NanoBananaProvider {
    ...
    async generateImage(input) {
        ...
        this.logger.log(`[NANO_BANANA] Creating task: model=${model} resolution=${resolution} aspectRatio=${input.aspectRatio ?? 'auto'} outputFormat=${input.outputFormat ?? 'png'} googleSearch=${input.googleSearch ?? false} imageUrls=${input.imageUrls?.length ?? 0} prompt="${input.prompt}"`);
