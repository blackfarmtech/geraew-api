...
var GenerationProcessingService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.GenerationProcessingService = void 0;
...
let GenerationProcessingService = GenerationProcessingService_1 = class GenerationProcessingService {
    prisma;
    creditsService;
    uploadsService;
    geraewProvider;
    ...
    generationEvents;
    promptEnhancer;
    logger = new common_1.Logger(GenerationProcessingService_1.name);
    ...
    async dispatch(jobName, data) {
        this.logger.log(`Processing job [${jobName}] for generation ${data.generationId}`);
        switch (jobName) {
            ...
            case generation_queue_constants_1.GenerationJobName.IMAGE_WITH_FALLBACK:
                return this.processImageWithFallback(data);
            case generation_queue_constants_1.GenerationJobName.IMAGE_NANO_BANANA:
                return this.processNanoBanana(data);
            case generation_queue_constants_1.GenerationJobName.TEXT_TO_VIDEO:
                return this.processTextToVideo(data);
            case generation_queue_constants_1.GenerationJobName.IMAGE_TO_VIDEO:
                return this.processImageToVideo(data);
            case generation_queue_constants_1.GenerationJobName.REFERENCE_VIDEO:
                return this.processReferenceVideo(data);
            case generation_queue_constants_1.GenerationJobName.MOTION_CONTROL:
                return this.processMotionControl(data);
            default:
                throw new Error(`Unknown job name: ${jobName}`);
        }
    }
    async onJobFailed(jobName, data, error, attemptsMade, maxAttempts) {
        this.logger.error(`Job [${jobName}] failed (attempt ${attemptsMade}/${maxAttempts}): ${error.message}`);
        if (attemptsMade < maxAttempts) {
            this.logger.warn(`Job [${jobName}] will be retried`);
            return;
        }
        await this.handleFailure(data.generationId, data.userId, data.creditsConsumed, error);
    }
    async processImage(data) {
        const startTime = Date.now();
        await this.markProcessingStarted(data.generationId);
        this.logger.log(`[processImage] gen=${data.generationId} model=${data.model} resolution=${data.resolution} aspectRatio=${data.aspectRatio} mimeType=${data.mimeType} hasInputImages=${data.hasInputImages} prompt="${data.prompt}"`);
    ...
    async processImageWithFallback(data) {
        const startTime = Date.now();
        await this.markProcessingStarted(data.generationId);
        this.logger.log(`[processImageWithFallback] gen=${data.generationId} model=${data.model} resolution=${data.resolution} aspectRatio=${data.aspectRatio} mimeType=${data.mimeType} hasInputImages=${data.hasInputImages} prompt="${data.prompt}"`);
        const images = data.hasInputImages
            ? await this.loadInputImagesAsBase64(data.generationId)
            : undefined;
        try {
    ...
    async processNanoBanana(data) {
        const startTime = Date.now();
        await this.markProcessingStarted(data.generationId);
        this.logger.log(`[processNanoBanana] gen=${data.generationId} model=${data.model} resolution=${data.resolution} aspectRatio=${data.aspectRatio} outputFormat=${data.outputFormat} googleSearch=${data.googleSearch} imageUrls=${data.imageUrls?.length ?? 0} prompt="${data.prompt}"`);
        const result = await this.nanoBananaProvider.generateImage({
            id: data.generationId,
            model: data.model,
            prompt: data.prompt,
    ...
    async processTextToVideo(data) {
        const startTime = Date.now();
        await this.markProcessingStarted(data.generationId);
        this.logger.log(`[processTextToVideo] gen=${data.generationId} model=${data.model} resolution=${data.resolution} duration=${data.durationSeconds}s aspectRatio=${data.aspectRatio} audio=${data.generateAudio} sampleCount=${data.sampleCount} negativePrompt="${data.negativePrompt ?? ''}" prompt="${data.prompt}"`);
    ...
    async processImageToVideo(data) {
        const startTime = Date.now();
        await this.markProcessingStarted(data.generationId);
        this.logger.log(`[processImageToVideo] gen=${data.generationId} model=${data.resolvedModel} resolution=${data.resolution} duration=${data.durationSeconds}s aspectRatio=${data.aspectRatio} audio=${data.generateAudio} sampleCount=${data.sampleCount} negativePrompt="${data.negativePrompt ?? ''}" prompt="${data.prompt}"`);
        const inputImages = await this.prisma.generationInputImage.findMany({
            where: { generationId: data.generationId },
            orderBy: { order: 'asc' },
        });
    ...
    async processReferenceVideo(data) {
        const startTime = Date.now();
        await this.markProcessingStarted(data.generationId);
        this.logger.log(`[processReferenceVideo] gen=${data.generationId} model=${data.resolvedModel} resolution=${data.resolution} duration=${data.durationSeconds}s aspectRatio=${data.aspectRatio} audio=${data.generateAudio} sampleCount=${data.sampleCount} negativePrompt="${data.negativePrompt ?? ''}" prompt="${data.prompt}"`);
        const inputImages = await this.prisma.generationInputImage.findMany({
            where: {
    ...
    async processMotionControl(data) {
        const startTime = Date.now();
        await this.markProcessingStarted(data.generationId);
        this.logger.log(`[processMotionControl] gen=${data.generationId} resolution=${data.resolution} videoUrl=${data.videoUrl} imageUrl=${data.imageUrl}`);
    ...
    async retryWithRefinedPrompt(generationId, originalPrompt, generateFn) {
        ...
        this.logger.log(`[retryWithRefinedPrompt] gen=${generationId} originalPrompt="${originalPrompt}" refinedPrompt="${refinedPrompt}"`);
        ...
        this.logger.log(`Retrying generation ${generationId} with refined prompt`);
        try {
            ...
            if (this.isSafetyRelatedError(retryError)) {
                throw new content_safety_error_1.ContentSafetyError('A imagem ou texto enviado viola nossas diretrizes de conteúdo mesmo após ajuste automático. Tente reformular sua ideia de forma diferente.', retryError instanceof content_safety_error_1.ContentSafetyError ? retryError.supportCode : undefined);
            }
            throw retryError;
        }
    }
    ...
    async handleFailure(generationId, userId, creditsConsumed, error) {
        ...
        const isSafetyError = error instanceof content_safety_error_1.ContentSafetyError;
...
exports.GenerationProcessingService = GenerationProcessingService;
exports.GenerationProcessingService = GenerationProcessingService = GenerationProcessingService_1 = __decorate([
...
//# sourceMappingURL=generation-processing.service.js.map