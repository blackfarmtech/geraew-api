...
let PromptEnhancerService = class PromptEnhancerService {
    static { PromptEnhancerService_1 = this; }
    configService;
    logger = new common_1.Logger(PromptEnhancerService_1.name);
    anthropic;
    static MAX_IMAGE_BYTES = 4.5 * 1024 * 1024;
    ...
    async compressImageForVision(base64Data) {
        const buffer = Buffer.from(base64Data, 'base64');
        this.logger.log(`[VISION] Compressing image for vision: ${(buffer.length / 1024 / 1024).toFixed(1)}MB input`);
        const compressed = await sharp(buffer)
            .resize(1024, 1024, { fit: 'inside', withoutEnlargement: true })
            .jpeg({ quality: 75 })
            .toBuffer();
        this.logger.log(`[VISION] Compressed to ${(compressed.length / 1024 / 1024).toFixed(1)}MB`);
        return {
            base64: compressed.toString('base64'),
            mime_type: 'image/jpeg',
        };
    }
    async enhance(prompt, context, images) {
        let textMessage = prompt;
        if (context) {
            const contextParts = [];
            contextParts.push(`[Generation type: ${context.type}]`);
            if (context.model)
                contextParts.push(`[Model: ${context.model}]`);
            if (context.resolution)
                contextParts.push(`[Resolution: ${context.resolution}]`);
            if (context.aspectRatio)
                contextParts.push(`[Aspect ratio: ${context.aspectRatio}]`);
            if (context.quality)
                contextParts.push(`[Quality: ${context.quality}]`);
            if (context.durationSeconds)
                contextParts.push(`[Duration: ${context.durationSeconds}s]`);
            if (context.hasAudio)
                contextParts.push(`[Has audio: yes]`);
            if (context.hasReferenceImages)
                contextParts.push(`[Has reference images: yes]`);
            if (context.hasFirstFrame)
                contextParts.push(`[Has first frame reference: yes]`);
            if (context.hasLastFrame)
                contextParts.push(`[Has last frame reference: yes]`);
            if (context.negativePrompt)
                contextParts.push(`[User negative prompt: ${context.negativePrompt}]`);
            if (context.sampleCount && context.sampleCount > 1)
                contextParts.push(`[Sample count: ${context.sampleCount}]`);
            textMessage = `${contextParts.join(' ')}\n\nUser prompt: ${prompt}`;
        }
        let userContent = textMessage;
        if (images && images.length > 0) {
            const compressedImages = await Promise.all(images.map((img) => this.compressImageForVision(img.base64)));
            userContent = [
    ...
    static SAFETY_REFINER_SYSTEM_PROMPT = `# System Prompt — Agente Refinador de Prompts para Veo 3.1 (Vertex AI)
...
- Se o prompt for muito curto/vago: Expanda com detalhes cinematográficos mantendo-se 100% fiel à intenção expressa — nunca adicione narrativa que o usuário não pediu.`;
    async refinePromptForSafety(originalPrompt) {
        this.logger.log(`[SAFETY REFINER] Refining prompt blocked by safety filters`);
        const response = await this.anthropic.messages.create({
            model: 'claude-sonnet-4-20250514',
            max_tokens: 1500,
            system: PromptEnhancerService_1.SAFETY_REFINER_SYSTEM_PROMPT,
            messages: [{ role: 'user', content: originalPrompt }],
            temperature: 0.7,
        });
        const content = response.content[0];
        const refinedPrompt = content.type === 'text' ? content.text.trim() : '';
        if (!refinedPrompt) {
            this.logger.warn('[SAFETY REFINER] Empty response from Claude');
            return null;
        }
        if (refinedPrompt === 'BLOCKED') {
            this.logger.warn('[SAFETY REFINER] Prompt is entirely prohibited content — cannot refine');
            return null;
        }
        this.logger.log(`[SAFETY REFINER] Refined prompt (${refinedPrompt.length} chars): ${refinedPrompt.substring(0, 100)}...`);
