...
let StripeWebhookService = StripeWebhookService_1 = class StripeWebhookService {
    ...
    async handleWebhook(payload, signature) {
        ...
        const log = await this.webhookLogsService.create('stripe', eventType, externalId, event);
        try {
    ...
    async handleCheckoutSessionCompleted(event) {
        this.logger.log(`Stripe checkout.session.completed: ${event.data?.object?.id ?? 'unknown'}`);
    }
    async handleInvoicePaymentSucceeded(event) {
        this.logger.log(`Stripe invoice.payment_succeeded: ${event.data?.object?.id ?? 'unknown'}`);
    }
    async handleInvoicePaymentFailed(event) {
        this.logger.log(`Stripe invoice.payment_failed: ${event.data?.object?.id ?? 'unknown'}`);
    }
    async handleSubscriptionDeleted(event) {
        this.logger.log(`Stripe customer.subscription.deleted: ${event.data?.object?.id ?? 'unknown'}`);
    }
};
